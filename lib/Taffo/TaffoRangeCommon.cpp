...
auto ::taffo::inferAssign<std::pair<APFloat, APFloat>(argRanges) {
  assert(argRanges[0].first <= argRanges[0].second);
  return argRanges[0];
}

auto ::taffo::inferAdd<std::pair<APFloat, APFloat>(argRanges) {
  assert(argRanges[0].first <= argRanges[0].second);
...
auto ::taffo::inferMult<std::pair<APFloat, APFloat>(argRanges) {
